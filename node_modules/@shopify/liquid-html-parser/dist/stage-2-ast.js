...
function isLiquidBranchDisguisedAsTag(node) {
    return node.type === types_1.NodeTypes.LiquidTag && ['else', 'elsif', 'when'].includes(node.name);
}
function isConcreteLiquidBranchDisguisedAsTag(node) {
    return node.type === stage_1_cst_1.ConcreteNodeTypes.LiquidTag && ['else', 'eslif', 'when'].includes(node.name);
}
...
class ASTBuilder {
    ...
    push(node) {
        if (node.type === types_1.NodeTypes.LiquidTag && isLiquidBranchDisguisedAsTag(node)) {
